        ...
        <>
          ...
          >
            <div className="p-margin-mobile flex justify-between items-center border-b border-outline-variant/30 h-20">
              <h3 className="font-serif text-2xl text-matte-black uppercase tracking-widest font-bold">LUMIÈRE</h3>
              <button onClick={onClose} className="p-2 hover:bg-matte-black/5 rounded-full transition-colors">
                <X size={24} />
              </button>
            </div>
            ...
            <nav className="flex-1 overflow-y-auto p-margin-mobile">
              ...
              <div className="mt-12 pt-12 border-t border-outline-variant/30 flex flex-col gap-4">
                <Link href="/contact" onClick={onClose} className="text-sm font-semibold uppercase tracking-widest text-on-surface-variant hover:text-deep-rose transition-colors">
                  Contact Us
                </Link>
                <Link href="/journal" onClick={onClose} className="text-sm font-semibold uppercase tracking-widest text-on-surface-variant hover:text-deep-rose transition-colors">
                  Journal
                </Link>
              </div>
            </nav>
...
export function Navbar({ onToggleSidebar }: { onToggleSidebar: () => void }) {
  ...
  const navLinkClass = "text-xs font-semibold uppercase tracking-[0.2em] relative py-2 group overflow-hidden";
  const navLinkHoverClass = "absolute bottom-0 left-0 w-full h-[1px] bg-gold -translate-x-full group-hover:translate-x-0 transition-transform duration-300";

  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-500 ${isScrolled ? 'bg-white shadow-md h-16' : 'bg-transparent h-24'}`}>
      <div className="flex justify-between items-center h-full px-margin-mobile md:px-margin-desktop max-w-7xl mx-auto">
        <div className="flex items-center gap-8">
          <button onClick={onToggleSidebar} className="text-on-surface p-1 hover:text-deep-rose transition-colors">
            <Menu size={24} />
          </button>
        ...
        <Link href="/" className="font-serif text-2xl md:text-3xl uppercase tracking-[0.3em] font-bold text-matte-black absolute left-1/2 -translate-x-1/2">
          LUMIÈRE
        </Link>
        ...
        <div className="flex items-center gap-4">
          ...
          <Link href="/cart" className="text-on-surface p-1 relative hover:text-deep-rose transition-colors">
            <ShoppingBag size={22} />
...
export function Footer() {
  ...
  const footerLinkClass = "text-sm text-warm-cream/60 hover:text-gold transition-colors duration-300";
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-12 mb-16">
          <div className="flex flex-col gap-4">
            ...
            <ul className="flex flex-col gap-3">
              <li><Link href="/contact" className={footerLinkClass}>Our Story</Link></li>
              <li><Link href="/contact" className={footerLinkClass}>Sustainability</Link></li>
              <li><Link href="/contact" className={footerLinkClass}>Journal</Link></li>
          ...
          <div className="flex flex-col gap-4">
            ...
            <ul className="flex flex-col gap-3">
              <li><Link href="/category/all" className={footerLinkClass}>Best Sellers</Link></li>
              <li><Link href="/category/skincare" className={footerLinkClass}>Skincare</Link></li>
              <li><Link href="/category/makeup" className={footerLinkClass}>Makeup</Link></li>
              <li><Link href="/category/fragrance" className={footerLinkClass}>Fragrance</Link></li>
            </ul>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <ul className="flex flex-col gap-3">
              <li><Link href="/contact" className={footerLinkClass}>Shipping & Delivery</Link></li>
              <li><Link href="/contact" className={footerLinkClass}>Returns & Exchanges</Link></li>
              <li><Link href="/contact" className={footerLinkClass}>Beauty Help</Link></li>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex gap-4 mb-4">
              <a href="#" className="p-2 border border-warm-cream/20 rounded-full hover:bg-warm-cream hover:text-matte-black transition-all">
                <Instagram size={20} />
              </a>
              <a href="#" className="p-2 border border-warm-cream/20 rounded-full hover:bg-warm-cream hover:text-matte-black transition-all">
                <Camera size={20} />
